...
const useColors = (): ColorProps => {
	...
	//
	const [cardBg, setCardBg] = React.useState<string>('#fff');
	const [cardRadius, setCardRadius] = React.useState<number>(4);
	//
	...
	React.useEffect(() => {
		if (data) {
			...
			setBtnColor(data?.basic?.btnColor || '#202020');
			setBtnTextColor(data?.basic?.btnTextColor || '#fff');
